...
// Curated U.S. macro release calendar sourced from FRED release dates
// plus the official Federal Reserve FOMC schedule.
// Reuses the same FRED API key as your existing /api/fred endpoint.
...
const RELEASES = [
  { id: 10, label: "CPI / Core CPI", short: "CPI / Core CPI", time: "8:30 AM", category: "Inflation", impact: "High", source: "BLS", officialUrl: "https://www.bls.gov/cpi/" },
...
// Official FOMC statement dates. The policy statement is normally released
// at 2:00 p.m. Eastern on the second day of each scheduled meeting.
const FOMC_DATES = {
...
async function fetchReleaseDates(releaseId) {
  ...
  const url = `${FRED_BASE}/release/dates?release_id=${releaseId}` +
    `&api_key=${encodeURIComponent(FRED_API_KEY)}` +
    `&file_type=json&include_release_dates_with_no_data=true&limit=10000&sort_order=desc`;

  const r = await fetch(url);
  const body = await r.json().catch(() => ({}));
  if (!r.ok || body.error_code) {
    throw new Error(body.error_message || `FRED release ${releaseId} HTTP ${r.status}`);
  }
...
function impactRank(x) {
  return x === "High" ? 0 : x === "Medium" ? 1 : 2;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const fomc = FOMC_DATES[year] || [];
    fomc.filter(d => isInMonth(d, year, month)).forEach(date => {
      events.push({
        id: `fomc-${date}`,
        date,
        time: "2:00 PM",
